function batchConvertOSLOnifti(imagesDir)
...
% patientBirthDate = [];
% clear MasterV
ImagesDirContent = dir(imagesDir);
iC = 0;
...
for iLL = 1:length(DynScanContent),
    ...
    subjectFolder = fullfile(conversionFolder,subjName);
    %
    SubjDirContent = dir(fullfile(imagesDir,DynScanContent(iLL).name));
    %
    for iK = 1:length(SubjDirContent),
        if SubjDirContent(iK).isdir & logical(strfind(SubjDirContent(iK).name,'PET_AC_Dyn')),
            DicomFilesACContent = dir(fullfile(imagesDir,...
                DynScanContent(iLL).name,SubjDirContent(iK).name,'*.dcm'));
            if isempty(DicomFilesACContent),
                error('wdfasdg');
            end
            
            ImConvertOptions.InputFiles.filesPath  = fullfile(imagesDir,...
